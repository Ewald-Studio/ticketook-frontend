        ...
        <b-col v-if="session.id">
            <b-row class="text-center mt-4">
                <template v-if="message">
                    <p style="padding: 0.5em 0.2em; background: #fff;" class="text-dark">{{ message }}</p>
                </template>
                <template v-else>
        ...
        fetchZoneInfo() {
            return axios.get(`/zone/${this.zone_id}/info/`)
                .then(response => {
                    ...
                    this.timers.zone = setInterval(this.checkForNewSession, ZONE_REFRESH_PERIOD)
                })
                .then(() => {
                    this.fetchLog()
